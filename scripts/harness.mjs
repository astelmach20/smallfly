...
const READ = ['steer_attr_L','steer_attr_R','steer_sweet_L','steer_sweet_R','steer_ferment_L','steer_ferment_R','steer_danger_L','steer_danger_R','MN_leg_L','MN_leg_R','wing_L','wing_R','proboscis_L','proboscis_R'];
...
for (let sd = 0; sd < seeds; sd++) {
  seedRand = 17 + sd * 101;
  const b = new Brain(graph, meta, 1000 + sd * 7919, meta.lif); b.setInputs(amb); b.run(300);
  ...
  for (let t = 0; t < secs; t += dt) {
    fly.hunger = 0.6; TUNE.probEatThresh = 1e9;
    ...
    fly.applyRates(rates); fly.update(dt, world, [fly], clock, wind);
  ...
  res.push({ seed: sd, meanDist: dsum / n, inPct: 100 * reached / n, tReach, final: Math.hypot(fly.x - garden.x, fly.y - garden.y), attrHz: (fly.attrRate || 0) * 1000 });
}
const avg = k => res.reduce((a, r) => a + (r[k] ?? 0), 0) / res.length;
console.log(JSON.stringify(over), `meanDist=${avg('meanDist').toFixed(2)} inPct=${avg('inPct').toFixed(0)} reachedN=${res.filter(r => r.tReach !== null).length}/${seeds} tReach=${res.map(r => r.tReach === null ? '-' : r.tReach.toFixed(0)).join(',')} attrHz=${avg('attrHz').toFixed(0)}`);
